...
app.get("/bookings", async function (req, res) {
  try {
    ...
    // Date range (allow From-only, To-only, or both)
    if (dateFrom) {
    ...
    if (q) {
      ...
      if (/^\d+$/.test(q)) {
        where.push("b.bookingId = ?");
        params.push(parseInt(q, 10));
      }

      // General text match across owner/pet/service fields
      where.push(`
        (
          o.firstName LIKE ?
          OR o.lastName LIKE ?
...
app.post("/bookings/create", async (req, res) => {
  try {
    ...
    const endTime =
      req.body.endTime && String(req.body.endTime).trim() !== "" ? req.body.endTime : null;
...
app.post("/bookings/:bookingId/update", async (req, res) => {
  try {
    ...
    // Update pet
    await dbConnection.query("UPDATE pets SET petName = ?, species = ? WHERE petId = ?", [
      petName,
      species,
      petId,
    ]);
...
app.post("/bookings/:bookingId/delete", async (req, res) => {
  try {
    ...
    const [result] = await dbConnection.query("DELETE FROM bookings WHERE bookingId = ?", [
      bookingId,
    ]);
